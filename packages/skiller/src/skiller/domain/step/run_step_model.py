...
def parse_run_step(raw_step: object) -> RunStep:
    ...
    return RunStep(step_id=step_id, step_type=step_type, body=body)


def find_run_step(raw_steps: object, step_id: str) -> tuple[int, RunStep]:
    parsed_steps = parse_run_steps(raw_steps)

    match_index = -1
    match_step: RunStep | None = None
...
def parse_run_steps(raw_steps: object) -> list[RunStep]:
    if not isinstance(raw_steps, list):
        raise ValueError("invalid steps list")

    parsed_steps: list[RunStep] = []
    seen_step_ids: set[str] = set()

    for raw_step in raw_steps:
        parsed = parse_run_step(raw_step)
