...
@dataclass(frozen=True)
class WhatsAppPairResetResult:
    reset: bool
    paired: bool
    stopped_pairing: bool
    home: str
    session_path: str
...
class WhatsAppPairService:
    ...
    def reset(self) -> WhatsAppPairResetResult:
        ...
        self._clear_bridge_runtime_state()
        self._clear_bridge_managed_state()

        return WhatsAppPairResetResult(
            reset=True,
            paired=False,
            stopped_pairing=stopped_pairing,
            home=str(self._home_dir()),
    ...
    def _bridge_runtime_state_file(self) -> Path:
        return self._state_dir() / "bridge-runtime.json"

    def _bridge_managed_state_file(self) -> Path:
    ...
    def _clear_bridge_runtime_state(self) -> None:
        state_file = self._bridge_runtime_state_file()
        if state_file.exists():
            state_file.unlink()

    def _clear_bridge_managed_state(self) -> None:
        state_file = self._bridge_managed_state_file()
        if state_file.exists():
            state_file.unlink()
