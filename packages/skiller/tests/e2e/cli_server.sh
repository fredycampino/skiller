...
while (( attempt < 20 )); do
  ...
  if start_output="$(PYTHONPATH=packages/skiller/src "${runtime_python}" -m skiller server start 2>/dev/null)"; then
    break
  fi
  attempt=$((attempt + 1))
done
...
status_output="$(
  PYTHONPATH=packages/skiller/src "${runtime_python}" -m skiller server status
)"
...
stop_output="$(
  PYTHONPATH=packages/skiller/src "${runtime_python}" -m skiller server stop
)"
