...
def test_read_skill_file_from_internal_agent_directory(tmp_path) -> None:  # noqa: ANN001
    agents_dir = tmp_path / "agents"
    agent_dir = agents_dir / "demo"
    agent_dir.mkdir(parents=True)
    (agent_dir / "agent.yaml").write_text(
    ...
    content = runner.read_skill_file("internal", "demo", "./system.md")

    assert content == "System prompt\n"


def test_read_skill_file_rejects_escape_from_skill_directory(tmp_path) -> None:  # noqa: ANN001
    agents_dir = tmp_path / "agents"
    agent_dir = agents_dir / "demo"
    agent_dir.mkdir(parents=True)
    (agent_dir / "agent.yaml").write_text(
    ...
    with pytest.raises(ValueError, match="escapes skill directory"):
        runner.read_skill_file("internal", "demo", "../secret.md")


def test_read_skill_file_from_file_skill_directory(tmp_path) -> None:  # noqa: ANN001
    skill_file = tmp_path / "external.yaml"
    skill_file.write_text(
        "name: external\nstart: support_agent\nsteps: []\n",
        encoding="utf-8",
    ...
    content = runner.read_skill_file("file", str(skill_file), "system.md")
