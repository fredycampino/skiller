...
class ExecuteWaitWebhookStepUseCase:
    ...
    def execute(self, current_step: CurrentStep) -> StepExecutionResult:
        ...
        webhook_event = self.store.get_latest_webhook_event(webhook, key, since_created_at=since_created_at)
        ...
        wait_id = str(active_wait["id"]) if active_wait is not None else self.store.create_wait(
            current_step.run_id,
            webhook,
            key,
            step_id=step_id,
