...
class _FakeLLM:
    ...
    def generate(self, messages: list[dict[str, str]], config: dict[str, object] | None = None) -> dict[str, object]:
        ...
        return {
            "ok": True,
            "content": '{"summary":"tests failing on auth","severity":"high","next_action":"retry"}',
            "model": "fake-llm",
        }
...
def test_llm_prompt_step_succeeds_and_persists_json_result() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        run_result = runtime.start_run(str(skill_path), {"stderr": "Traceback auth failed"}, skill_source="file")
...
def test_llm_prompt_step_succeeds_with_fake_llm_provider_from_container() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        container.bootstrap_service.initialize()
        assert isinstance(container.runtime_service.execute_llm_prompt_step_use_case.llm, FakeLLM)

        run_result = container.runtime_service.start_run("llm_prompt", {"stderr": "boom"})
...
def test_llm_prompt_failure_persists_debug_info_in_run_failed_event() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        run_result = container.runtime_service.start_run("llm_prompt", {"issue": "boom"})
        run = container.query_service.get_run_status_use_case.execute(run_result["run_id"])
        events = container.query_service.get_run_logs_use_case.execute(run_result["run_id"])
        ...
        assert failed_event["payload"]["error"] == "LLM step 'start' returned invalid JSON: Expecting value"
