...
def test_start_cancels_pair_cleanly_on_keyboard_interrupt(
    ...
    monkeypatch.setattr(service, "_ensure_bridge_requirements", lambda: None)
    monkeypatch.setattr(
        "skiller.local.channels.whatsapp.pair_service.subprocess.Popen",
...
def test_reset_deletes_local_session_and_state(monkeypatch: pytest.MonkeyPatch) -> None:
    service = WhatsAppPairService(_settings())
    stopped: list[int] = []
    service._session_dir().mkdir(parents=True, exist_ok=True)
    (service._session_dir() / "creds.json").write_text("{}", encoding="utf-8")
    service._write_pair_state(1234)
    service._runtime_state_file().write_text("{}", encoding="utf-8")
    service._bridge_runtime_state_file().write_text("{}", encoding="utf-8")
    service._bridge_managed_state_file().write_text("{}", encoding="utf-8")
    ...
    assert result.paired is False
    assert result.stopped_pairing is True
    assert stopped == [1234]
    assert service._session_dir().exists()
    assert not (service._session_dir() / "creds.json").exists()
    assert not service._pair_state_file().exists()
    assert not service._runtime_state_file().exists()
    assert not service._bridge_runtime_state_file().exists()
    assert not service._bridge_managed_state_file().exists()
