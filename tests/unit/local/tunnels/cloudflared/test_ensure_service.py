...
def test_ensure_requires_authenticated_login(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "skiller.local.tunnels.cloudflared.ensure_service.CloudflaredLoginService",
        _FakeLoginService,
    )
...
def test_create_tunnel_falls_back_to_uuid_in_output(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("skiller.local.tunnels.cloudflared.ensure_service.subprocess.run", fake_run)
